# -*- coding:utf-8 -*-
# RANGED_PREPROCESSOR의 결과를 바탕으로, 주차별 리뷰 가중합과, 주차별 리뷰 수를 나타냄
import os
...
def count_reviews_per_week(week_list):
    ...
    count_list = [0] * NUM_OF_LIST_ENTRY
...
def get_num_past_weeks(dataframe):
    ...
    for review_date in dataframe['Days_of_overall']:
        ...
        # 연도 차이가 나는 경우도 고려함
        # 20190722.0 처럼 float 형식일 수도 있음. int로 캐스팅 후 str로 캐스팅
        converted_date = datetime.strptime(str(int(review_date)), '%Y%m%d').date()
...
if __name__ == '__main__':
    ...
    # START_DATE부터 현재 일자까지를 종합
    NUM_OF_LIST_ENTRY = datetime.today().isocalendar()[1] - datetime.strptime(START_DATE, '%Y%m%d').date().isocalendar()[1]\
                        + 1  # 이번 주도 포함해서 세야 하므로 +1
    ...
    pd.DataFrame(data=productwise_number_of_reviews_over_weeks_and_weighted_sum, \
                 columns=['Product_name'] + [('Week' + str(_)) for _ in range(0, NUM_OF_LIST_ENTRY)] + ['Weight_sum']) \
                 .to_csv(weighted_sum_directory + '\\' + 'PRODUCTWISE_' + today.strftime('%Y-%m-%d') + '.csv', index=False, encoding='utf-8')
    print('Productwise weighted num of reviews -> complete.')
