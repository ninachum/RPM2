...
# 크롤링 데이터가 주어졌을 때, 범위가 주어진(START_DATE ~ END_DATE [[in flip_global_variables.py]]) 리뷰 일자에 대해 전처리함.
# 단, 리뷰 일자가 'month ago' 단위인 경우 무시함. (누적된 크롤링과 전처리로 정확한 일자들을 모을 수 있다는 가정 하)
# 따라서, 1 month ago와 같은 리뷰의 일자를 랜덤화 하지 않고, 정확한 일자로 누적해나가는데 사용
...
# 일자별로 정렬된 source data 가정함
# 크롤링 일자 기준 1달 이상 지난 경우 고려하지 않음(크롤링 데이터 누적으로 정확한 리뷰 일자들을 추출할 수 있을 것)
# TODO: 성능 최적화
def convert_to_absolute_data_and_filter_old_date(dataframe, filename):
    ...
    for ago_date in dataframe['Days_of_overall']:
        if 'Today' in ago_date:
            # RANGE
            absolute_date = india_local_date
            # SS 보고용 임시 코드
            # if absolute_date >= datetime.strptime(END_DATE, '%Y%m%d').date() + relativedelta(days=1):
            #     absolute_date = datetime.strptime(END_DATE, '%Y%m%d').date()

            # print(absolute_date)
            # RANGE
            if absolute_date >= datetime.strptime(START_DATE, '%Y%m%d').date() and absolute_date <= datetime.strptime(END_DATE, '%Y%m%d').date():
                up_to_date_list.append(convert_date_to_string(absolute_date))
            else:
                up_to_date_list.append(NaN)
            # print(up_to_date_list[-1])
        elif 'day' in ago_date:
            absolute_date = india_local_date - relativedelta(days=int(''.join(filter(str.isdigit, ago_date))))
            # SS 보고용 임시 코드
            # if absolute_date >= datetime.strptime(END_DATE, '%Y%m%d').date() + relativedelta(days=1):
            #     absolute_date = datetime.strptime(END_DATE, '%Y%m%d').date()
            ...
            if absolute_date >= datetime.strptime(START_DATE, '%Y%m%d').date() and absolute_date <= datetime.strptime(END_DATE, '%Y%m%d').date():
                ...
                up_to_date_list.append(NaN)
        else:  # 크롤링 일자 기준 1달 이상 지난 경우 고려하지 않음(크롤링 데이터 누적으로 정확한 리뷰 일자들을 추출할 수 있을 것)
            break

    # 패딩하여 dataframe 오류 방지
    while len(up_to_date_list) < len(dataframe['Days_of_overall']):
        up_to_date_list.append(NaN)
    dataframe['Days_of_overall'] = up_to_date_list
...
if __name__ == '__main__':
    ...
    # 빈 파일이 주어진 경우가 있음. 체크리스트로 마지막에 출력해서 사용자에게 확인 요구
    ...
    for filename_with_dir in filename_with_dir_list:
        ...
        right = convert_to_absolute_data_and_filter_old_date(right, filename)
        ...
        try:
            total = pd.concat([total, right], sort=False).drop_duplicates(subset=column_name_list_for_drop_duplicate)
    ...
    # 전처리된 파일을 정렬하여 저장
    preprocessed_filename_list = os.listdir(dest_directory)
    for preprocessed_filename in preprocessed_filename_list:
        to_be_sorted = pd.read_csv(dest_directory + '\\' + preprocessed_filename,
                                   usecols=column_name_list, encoding='UTF-8')
        to_be_sorted.sort_values('Days_of_overall', inplace=True, ascending=False)
        to_be_sorted.to_csv(dest_directory + '\\' + preprocessed_filename, encoding='UTF-8', index=False)
